...
<body class="sb-nav-fixed">
    ...
    if ($_SESSION['role'] !== 'admin_htg' && $_SESSION['role'] !== 'supply_htg' && $_SESSION['role'] !== 'logistik_htg') {
            ...
            <main>
                ...
                <div class="container-fluid">
                    ...
                    <div class="">
                            ...
                            <h1 class="mt-3 text-center mb-4" style="color:#1E7458;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">PERMINTAAN BARANG</h1>
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <select name="supplier2" class="form-control" required>
                                                                ...
                                                                $ambilsemuadata = mysqli_query($conn_htg, "select * from supplier");
...
if(isset($_POST['tambahpermintaan'])) {
    ...
    foreach($tanggalbuat as $key => $value) {
        ...
        $getSupplierInfo = $conn_htg->prepare("SELECT * FROM supplier WHERE idsupplier = ?");
        ...
        $getbaranginfo = $conn_htg->prepare("SELECT * FROM stok WHERE idbarang = ?");
        ...
        $kodeTransaksi[$key] = generateUniqueTransactionCode($conn_htg);
...
if (isset($_POST['updatepermintaan'])) {
    ...
    $getSupplierInfo = $conn_htg->prepare("SELECT namasupplier FROM supplier WHERE idsupplier = ?");
    ...
    $cekstoksekarang = $conn_htg->prepare("SELECT namabarang FROM stok WHERE idbarang = ?");
    ...
    $updatepermintaan = $conn_htg->prepare("UPDATE permintaan SET tanggal_buat=?, tanggal_permintaan=?, item_barang=?, qty=?, satuan_bentuk=?, harga_satuan=?, ppn=?, diskon=?, total_harga=?, idsupplier=?, nama_supplier_order=?, keterangan=?, user_edit_order=? WHERE id_order=?");
...
if (isset($_POST['hapuspermintaan'])) {
    ...
    $delete_query2 = $conn_htg->prepare("DELETE FROM persetujuan_request WHERE idorder = ?");
    ...
    if ($delete_result2) {
        ...
        $delete_query = $conn_htg->prepare("DELETE FROM permintaan WHERE id_order = ?");
