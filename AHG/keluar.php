                ...
                <div class="container-fluid">
                    ...
                    <div class=" mb-4">
                        ...
                        <div class="">
                            ...
                            <h1 class="mt-3 text-center mb-4" style="color:#DD5555;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">BARANG KELUAR</h1>
                                    ...
                                    <tbody>
                                        ...
                                        if(isset($_GET['cari'])){ 
                                            ...
                                            if($mulai != null && $selesai != null){
                                                $ambilsemuadata = mysqli_query($conn_ahg, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY) ORDER BY k.kode_transaksi ASC");
                                            } elseif($mulai != null && $selesai == null) {
                                                $ambilsemuadata = mysqli_query($conn_ahg, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar ='$mulai' ORDER BY k.kode_transaksi ASC");
                                            } else {
                                                $ambilsemuadata = mysqli_query($conn_ahg, "select * from keluar k, stok s where s.idbarang = k.idbarang ORDER BY k.kode_transaksi ASC");
                                            }
                                        } else {
                                            $ambilsemuadata = mysqli_query($conn_ahg, "select * from keluar k, stok s where s.idbarang = k.idbarang ORDER BY k.kode_transaksi ASC");
                                        }
                                                ...
                                                <?php
                                                    ...
                                                    if ($status == 'IN PROGRESS') {
                                                        ...
                                                        <div class="btn-group">
                                                            ...
                                                            <button type="button" class="btn dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-expanded="false" style="background-color:#DD5555; height:30px; font-size:12px; color:white;">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="barangnya[]" class="form-control" required>
                                        ...
                                        $ambilsemuadatanya = mysqli_query($conn_ahg, "select * from stok");
...
function sendEmail($emailRecipients, $subject, $body) {
    ...
    try {
        ...
        $mail->setFrom('mspasyazakaria@gmail.com', 'PT Amanah Harun Gasindo'); 
...
if (isset($_POST['barangkeluar'])) {
    ...
    function generateUniqueTransactionCode($conn_ahg) {
        $tahun = date('Y');
        $bulan = date('m');
    ...
    for ($i = 0; $i < count($barangnya); $i++) {
        ...
        $stmt_cek_stok = $conn_ahg->prepare($query_cek_stok);
        ...
        if ($result_stok->num_rows > 0) {
            ...
            if ($stoksekarang >= $qty_barang) {
                ...
                $kodeTransaksi = generateUniqueTransactionCode($conn_ahg);
                ...
                $ambilUsername = mysqli_prepare($conn_ahg, "SELECT username, iduser FROM login WHERE role = ?");
                ...
                $stmt_add_keluar = $conn_ahg->prepare($query_add_keluar);
                ...
                $stmt_update_stok = $conn_ahg->prepare($query_update_stok);
...
if (isset($_POST['updatebarangkeluar'])) {
    ...
    $lihatstok = $conn_ahg->prepare("SELECT jmlhstok FROM stok WHERE idbarang = ?");
    ...
    $lihatqty = $conn_ahg->prepare("SELECT jumlah FROM keluar WHERE idkeluar = ?");
    ...
    $query_get_barang = $conn_ahg->prepare("SELECT namabarang, dokumentasi FROM stok WHERE idbarang = ?");
    ...
    if ($stoksebelumnya >= $selisih_qty) {
        ...
        $update_stok = $conn_ahg->prepare("UPDATE stok SET jmlhstok = ? WHERE idbarang = ?");
        ...
        $update_keluar = $conn_ahg->prepare("UPDATE keluar SET jumlah = ?, penerima = ?, tanggal_keluar = ?, units = ?, keterangank = ?, namabarang = ?, keperluan = ?, status = ?, user_edit_keluar = ? WHERE idkeluar = ?");
...
if (isset($_POST['hapusbarangkeluar'])) {
    ...
    $status_query = $conn_ahg->prepare("select keperluan from keluar where idkeluar = ?");
    ...
    if ($status_keperluan == 'EXTERNAL'){
        ...
        $delete_query2 = $conn_ahg->prepare("DELETE FROM persetujuan_keluar WHERE id_keluar = ?");
        ...
        if ($delete_result2){
            ...
            $hapus = mysqli_query($conn_ahg, "DELETE FROM keluar WHERE idkeluar='$idk'");
