...
$get = mysqli_prepare($conn_mes, "SELECT * FROM stok WHERE idbarang=?");
...
$get_karyawan = mysqli_prepare($conn_mes, "SELECT * FROM karyawan WHERE idkaryawan = ?");
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    ...
                    <h1 class="mt-3 text-center mb-4" style="color:#4045AA; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);"><?=$namabarang;?></h1>
                        <div class="card-body">
                            <div class="row mt-5">
                                <h4 style="color:#4045AA; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">Detail Barang</h4>
                            </div>
                            ...
                            <div class="row justify-content-center mt-5" id="foto_lokasi">
                                <div class="col-md-6">
                                    <h4 style="color:#4045AA;">FOTO BARANG/ LOKASI BARANG :</h4>
                                </div>    
                                <div class="col-md-6">
                            ...
                            <hr>
                            <div class="col-md-12">
                                <h4 style="color:#4045AA;">DATA BARANG MASUK :</h4>
                            </div>   
                            <!-- TABLE BARANG MASUK -->
                            <div class="table-responsive shadow-lg px-1 py-5">
                                <table class="table table-bordered text-center" id="dataTable" width="" cellspacing="0" style="font-size:11px; text-transform: uppercase;">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                        $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM masuk m JOIN stok s ON s.idbarang = m.idbarang WHERE m.idbarang = '$idb'");
                                        $i= 1;
                            ...
                            <hr>
                            <div class="col-md-12">
                                <h4 style="color:#4045AA;">DATA BARANG KELUAR :</h4>
                            </div>   
                            <!-- TABLE BARANG KELUAR -->
                            <div class="table-responsive shadow-lg px-1 py-5">
                                <table class="table table-bordered text-center" id="dataTableKeluar" width="" cellspacing="0" style="font-size:11px; text-transform: uppercase;">
                                    ...
                                    <tbody>
                                        ...
                                        <?php
                                        $i= 1;
                                        $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON k.idbarang = s.idbarang WHERE k.idbarang = '$idb' ORDER BY k.kode_transaksi ASC");
                                        while ($data = mysqli_fetch_array($ambilsemuadata)) {
                                            $idk = $data['idkeluar'];
                                            $idb = $data['idbarang'];
...
if (isset($_POST['submit_dokumen_barang'])) {
    ...
    foreach ($imageFiles2['tmp_name'] as $key2 => $tmp_name) {
        ...
        if (!empty($imageFileName2)) {
            ...
            $stmt = mysqli_prepare($conn_mes, $sqlImage);
...
if (isset($_POST['hapus_gambar_lokasi'])) {
    ...
    $gambarQuery = mysqli_query($conn_mes, "SELECT * FROM dokumen_lokasi_barang WHERE id_lok_barang='$id_lok_barang'");
    ...
    if ($get) {
        ...
        if (unlink($lokgambar)) {
            $hapusgambar = mysqli_query($conn_mes, "DELETE FROM dokumen_lokasi_barang WHERE id_lok_barang='$id_lok_barang'");
            if ($hapusgambar) {
                echo '<script type="text/javascript">      
                        Swal.fire({
                            position: "center",
