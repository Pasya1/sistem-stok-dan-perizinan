...
<body class="sb-nav-fixed">
    ...
    if ($_SESSION['role'] !== 'admin_mes' && $_SESSION['role'] !== 'supply_mes' && $_SESSION['role'] !== 'logistik_mes') {
            ...
            <main>
                ...
                <div class="container-fluid">
                    ...
                    <div class="">
                            ...
                            <h1 class="mt-3 text-center mb-4" style="color:#4045AA;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">PERMINTAAN BARANG</h1>
                                    ...
                                    if(isset($_GET['cari'])){ 
                                        ...
                                        if($mulai != null && $selesai != null && $tgl_apa != null){
                                            if($tgl_apa == 'Tanggal_Buat') {
                                                $ambildatapermintaan = mysqli_query($conn_mes, "SELECT * from permintaan WHERE tanggal_buat BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY)");
                                            } elseif($tgl_apa == 'Tanggal_Permintaan') {
                                                $ambildatapermintaan = mysqli_query($conn_mes, "SELECT * from permintaan WHERE tanggal_permintaan BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY)");
                                            } else {
                                                $ambildatapermintaan = mysqli_query($conn_mes, "select * from permintaan");
                                            }
                                        } elseif($mulai != null && $selesai == null && $tgl_apa != null) {
                                            if($tgl_apa == 'Tanggal_Buat') {
                                                $ambildatapermintaan = mysqli_query($conn_mes, "SELECT * from permintaan WHERE tanggal_buat = '$mulai'");
                                            } elseif($tgl_apa == 'Tanggal_Permintaan') {
                                                $ambildatapermintaan = mysqli_query($conn_mes, "SELECT * from permintaan WHERE tanggal_permintaan = '$mulai'");
                                            } else {
                                                $ambildatapermintaan = mysqli_query($conn_mes, "select * from permintaan");
                                            }
                                        } else {
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <select name="barangnya" class="form-control" required>
                                                                ...
                                                                $ambilsemuadatanya = mysqli_query($conn_mes, "select * from stok");
                                                            ...
                                                            <select name="supplier2" class="form-control" required>
                                                                ...
                                                                $ambilsemuadata = mysqli_query($conn_mes, "select * from supplier");
...
function sendEmail($emailRecipients, $subject, $body) {
    ...
    try {
        ...
        $mail->setFrom('mspasyazakaria@gmail.com', 'PT Mitra Energi Selaras'); 
...
if(isset($_POST['tambahpermintaan'])) {
    ...
    function generateUniqueTransactionCode($conn_mes) {
        $tahun = date('Y');
        $bulan = date('m');
    ...
    foreach($tanggalbuat as $key => $value) {
        ...
        $getSupplierInfo = $conn_mes->prepare("SELECT * FROM supplier WHERE idsupplier = ?");
        ...
        $getbaranginfo = $conn_mes->prepare("SELECT * FROM stok WHERE idbarang = ?");
        ...
        $kodeTransaksi[$key] = generateUniqueTransactionCode($conn_mes);
...
if (isset($_POST['updatepermintaan'])) {
    ...
    $getSupplierInfo = $conn_mes->prepare("SELECT namasupplier FROM supplier WHERE idsupplier = ?");
    ...
    $cekstoksekarang = $conn_mes->prepare("SELECT namabarang FROM stok WHERE idbarang = ?");
    ...
    $updatepermintaan = $conn_mes->prepare("UPDATE permintaan SET tanggal_buat=?, tanggal_permintaan=?, item_barang=?, qty=?, satuan_bentuk=?, harga_satuan=?, ppn=?, diskon=?, total_harga=?, idsupplier=?, nama_supplier_order=?, keterangan=?, user_edit_order=? WHERE id_order=?");
...
if (isset($_POST['hapuspermintaan'])) {
    ...
    $delete_query2 = $conn_mes->prepare("DELETE FROM persetujuan_request WHERE idorder = ?");
    ...
    if ($delete_result2) {
        ...
        $delete_query = $conn_mes->prepare("DELETE FROM permintaan WHERE id_order = ?");
