...
if ($_SESSION['role'] !== 'visitor_mes' && $_SESSION['role'] !== 'owner_mes' && $_SESSION['role'] !== 'keuangan_mes' && $_SESSION['role'] !== 'management_mes' && $_SESSION['role'] !== 'ketua_operasional_mes') {
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    <div class=" mb-4">
                        <div class="" >
                        <h1 class="mt-3 text-center mb-4" style="color:#4045AA;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">PROFILE</h1>
                        </div>
                        <div class="row d-flex justify-content-around mt-5">
                            <div class="col-md-5  shadow-lg px-5 py-5">
                                        ...
                                        $ambiluser = mysqli_query($conn_mes, "select * from login where role='$role'");
                                        while ($row = mysqli_fetch_assoc($ambiluser)){
                                                $username = $row['username'];
                                                $password = $row['password'];

                                        ?>
                                        <h5 style="color:#4045AA;  text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.2);">Informasi Profil</h5><br>
                                        <form method="post">
                                            Username :<br>
                            ...
                            <div class="col-md-5 shadow-lg px-5 py-5">
                                    <h5 style="color:#4045AA;  text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.2);">Ganti Password</h5><br>
                                    <form method="post">
...
if (isset($_POST['updatepassword'])) {
    ...
    $ambilpw = mysqli_prepare($conn_mes, "SELECT * FROM login WHERE role = ?");
    ...
    if (password_verify($password_lama, $password_dari_database)) {
        if ($password_baru === $konfirmasi_password_baru) {
            ...
            $update_password_query = mysqli_prepare($conn_mes, "UPDATE login SET password = ? WHERE iduser = ?");
...
if (isset($_POST['perbarui'])) {
    ...
    $stmt = mysqli_prepare($conn_mes, $updateQuery);
