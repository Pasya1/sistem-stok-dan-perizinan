...
<body class="sb-nav-fixed">
    ...
    <?php
    if ($_SESSION['role'] !== 'management_mes' && $_SESSION['role'] !== 'ketua_operasional_mes') {
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    <div class=" mb-4">
                        <div class="">
                            <h1 class="mt-3 text-center mb-4" style="color:#4045AA;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">APPROVAL BARANG KELUAR</h1>
                        </div>
                        <div class="">
                                    ...
                                    <tbody>
                                        ...
                                        if(isset($_GET['cari'])){ 
                                            ...
                                            if($mulai != null && $selesai != null){
                                                $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY) AND k.status = 'IN PROGRESS' ORDER BY k.kode_transaksi ASC");
                                            } elseif($mulai != null && $selesai == null) {
                                                $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar ='$mulai' AND k.status = 'IN PROGRESS' ORDER BY k.kode_transaksi ASC");
                                            } else {
                                                $ambilsemuadata = mysqli_query($conn_mes, "select * from keluar k, stok s where s.idbarang = k.idbarang AND k.status = 'IN PROGRESS' ORDER BY k.kode_transaksi ASC");
                                            }
                                        } else {
                                            $ambilsemuadata = mysqli_query($conn_mes, "select * from keluar k, stok s where s.idbarang = k.idbarang AND k.status = 'IN PROGRESS' ORDER BY k.kode_transaksi ASC");
                                        }
                                ...
                                <table class="table table-bordered text-center" id="dataTable2" width="" cellspacing="0" style="font-size:11px; text-transform: uppercase;">
                                    ...
                                    <tbody>
                                        ...
                                        if(isset($_GET['cari'])){ 
                                            ...
                                            if($mulai != null && $selesai != null){
                                                $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY) AND k.status = 'ACCEPTED' ORDER BY k.kode_transaksi ASC");
                                            } elseif($mulai != null && $selesai == null) {
                                                $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar ='$mulai' AND k.status = 'ACCEPTED' ORDER BY k.kode_transaksi ASC");
                                            } else {
                                                $ambilsemuadata = mysqli_query($conn_mes, "select * from keluar k, stok s where s.idbarang = k.idbarang AND k.status = 'ACCEPTED' ORDER BY k.kode_transaksi ASC");
                                            }
                                        } else {
                                            $ambilsemuadata = mysqli_query($conn_mes, "select * from keluar k, stok s where s.idbarang = k.idbarang AND k.status = 'ACCEPTED' ORDER BY k.kode_transaksi ASC");
                                        }
                                ...
                                <table class="table table-bordered text-center" id="dataTable3" width="" cellspacing="0" style="font-size:11px; text-transform: uppercase;">
                                    ...
                                    <tbody>
                                        ...
                                        if(isset($_GET['cari'])){ 
                                            ...
                                            if($mulai != null && $selesai != null){
                                                $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY) AND k.status = 'REJECTED' ORDER BY k.kode_transaksi ASC");
                                            } elseif($mulai != null && $selesai == null) {
                                                $ambilsemuadata = mysqli_query($conn_mes, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar ='$mulai' AND k.status = 'REJECTED' ORDER BY k.kode_transaksi ASC");
                                            } else {
                                                $ambilsemuadata = mysqli_query($conn_mes, "select * from keluar k, stok s where s.idbarang = k.idbarang AND k.status = 'REJECTED' ORDER BY k.kode_transaksi ASC");
                                            }
                                        } else {
                                            $ambilsemuadata = mysqli_query($conn_mes, "select * from keluar k, stok s where s.idbarang = k.idbarang AND k.status = 'REJECTED' ORDER BY k.kode_transaksi ASC");
                                        }
    ...
    if (isset($_POST['approve']) || isset($_POST['reject'])) {
        ...
        if ($approvals_count >= 2 || $rejects_count >= 2) {
            ...
            if ($result_get_status) {
                ...
                $stmt_qty = $conn_mes->prepare($query_qty);
                ...
                if ($current_status == 'IN PROGRESS') {
                    if ($aksi == 'approve') { // Menggunakan $aksi untuk menentukan tindakan
                        ...
                        $stmt_stok = $conn_mes->prepare($query_stok);
                        ...
                        $updatestok = mysqli_query($conn_mes, "UPDATE stok SET jmlhstok='$qtypashapus' WHERE idbarang='$idb'");
