...
if (isset($_POST['barangkeluar'])) {
    ...
    for ($i = 0; $i < count($barangnya); $i++) {
        ...
        $stmt_cek_stok = $conn_mes->prepare($query_cek_stok);
        ...
        if ($result_stok->num_rows > 0) {
            ...
            if ($stoksekarang >= $qty_barang) {
                ...
                $stmt_add_keluar = $conn_mes->prepare($query_add_keluar);
                ...
                $stmt_update_stok = $conn_mes->prepare($query_update_stok);
...
if (isset($_POST['updatebarangkeluar'])) {
    ...
    $lihatstok = $conn_mes->prepare("SELECT jmlhstok FROM stok WHERE idbarang = ?");
    ...
    $lihatqty = $conn_mes->prepare("SELECT jumlah FROM keluar WHERE idkeluar = ?");
    ...
    $query_get_barang = $conn_mes->prepare("SELECT namabarang, dokumentasi FROM stok WHERE idbarang = ?");
    ...
    if ($stoksebelumnya >= $selisih_qty) {
        ...
        $update_stok = $conn_mes->prepare("UPDATE stok SET jmlhstok = ? WHERE idbarang = ?");
        ...
        $update_keluar = $conn_mes->prepare("UPDATE keluar SET jumlah = ?, penerima = ?, tanggal_keluar = ?, units = ?, keterangank = ?, namabarang = ?, keperluan = ?, status = ?, user_edit_keluar = ? WHERE idkeluar = ?");
...
if (isset($_POST['hapusbarangkeluar'])) {
    ...
    $status_query = $conn_mes->prepare("select keperluan from keluar where idkeluar = ?");
    ...
    if ($status_keperluan == 'EXTERNAL'){
        ...
        $delete_query2 = $conn_mes->prepare("DELETE FROM persetujuan_keluar WHERE id_keluar = ?");
        ...
        if ($delete_result2){
            ...
            $hapus = mysqli_query($conn_mes, "DELETE FROM keluar WHERE idkeluar='$idk'");
