                ...
                <div class="container-fluid">
                    ...
                    <div class=" mb-4">
                        ...
                        <div class="">
                            ...
                            <h1 class="mt-3 text-center mb-4" style="color:#427D9D;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">BARANG KELUAR</h1>
                            ...
                            <button type="button" class="btn btn-primary mb-3 shadow" data-toggle="modal" data-target="#myModal" style=" float: right;">
                                    ...
                                    <tbody>
                                        ...
                                        if(isset($_GET['cari'])){ 
                                            ...
                                            if($mulai != null && $selesai != null){
                                                $ambilsemuadata = mysqli_query($conn, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY) ORDER BY k.kode_transaksi ASC");
                                            } elseif($mulai != null && $selesai == null) {
                                                $ambilsemuadata = mysqli_query($conn, "SELECT * FROM keluar k JOIN stok s ON s.idbarang = k.idbarang WHERE k.tanggal_keluar ='$mulai' ORDER BY k.kode_transaksi ASC");
                                            } else {
                                                $ambilsemuadata = mysqli_query($conn, "select * from keluar k, stok s where s.idbarang = k.idbarang ORDER BY k.kode_transaksi ASC");
                                            }
                                        } else {
                                            $ambilsemuadata = mysqli_query($conn, "select * from keluar k, stok s where s.idbarang = k.idbarang ORDER BY k.kode_transaksi ASC");
                                        }
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <button type="submit" class="btn btn-primary" name="updatebarangkeluar" style="float: right;">Submit <i class="fas fa-arrow-circle-right"></i></button>
        ...
        <div class="modal-content">
            ...
            <form method="post" >
                ...
                <div class="barang-form">
                    ...
                    <button type="submit" class="btn btn-primary form-control mt-3" name="barangkeluar">Submit <i class="fas fa-arrow-circle-right"></i></button>
...
function sendEmail($emailRecipients, $subject, $body) {
    ...
    try {
        ...
        $mail->setFrom('mspasyazakaria@gmail.com', 'PT Mitra Harun Gasindo'); 
...
if (isset($_POST['barangkeluar'])) {
    ...
    //     // Query untuk menghitung jumlah transaksi untuk bulan ini
    //     $query = "SELECT COUNT(*) AS no_transaksi FROM keluar WHERE YEAR(waktu_terakhir_aksi_keluar) = $tahun AND MONTH(waktu_terakhir_aksi_keluar) = $bulan";
    ...
    //     // Mengambil jumlah transaksi untuk bulan ini
    //     $jumlah_transaksi = $row['no_transaksi'];
    
    //     // Membuat nomor transaksi baru
    //     $newNumber = $jumlah_transaksi + 1;
    ...
    for ($i = 0; $i < count($barangnya); $i++) {
        ...
        $stmt_cek_stok = $conn->prepare($query_cek_stok);
        ...
        if ($result_stok->num_rows > 0) {
            ...
            if ($stoksekarang >= $qty_barang) {
                ...
                $stmt_add_keluar = $conn->prepare($query_add_keluar);
...
if (isset($_POST['updatebarangkeluar'])) {
    ...
    $lihatstok = $conn->prepare("SELECT jmlhstok FROM stok WHERE idbarang = ?");
    ...
    $lihatqty = $conn->prepare("SELECT jumlah FROM keluar WHERE idkeluar = ?");
    ...
    $query_get_barang = $conn->prepare("SELECT namabarang, dokumentasi FROM stok WHERE idbarang = ?");
    ...
    if ($stoksebelumnya >= $selisih_qty) {
        ...
        $stok_baru = (int)$stoksebelumnya - $selisih_qty;



        // Update stock using prepared statement

        $update_stok = $conn->prepare("UPDATE stok SET jmlhstok = ? WHERE idbarang = ?");
        ...
        $update_keluar = $conn->prepare("UPDATE keluar SET jumlah = ?, penerima = ?, tanggal_keluar = ?, units = ?, keterangank = ?, namabarang = ?, keperluan = ?, status = ?, user_edit_keluar = ? WHERE idkeluar = ?");
...
if (isset($_POST['hapusbarangkeluar'])) {
    ...
    $status_query = $conn->prepare("select keperluan from keluar where idkeluar = ?");
    ...
    if ($status_keperluan == 'EXTERNAL'){
        ...
        $delete_query2 = $conn->prepare("DELETE FROM persetujuan_keluar WHERE id_keluar = ?");
