...
if (@$_SESSION['role'] == 'admin') {
    ...
    exit;
} elseif (@$_SESSION['role'] == 'logistik') {
    header('Location: index.php');
    exit;
} elseif (@$_SESSION['role'] == 'legal') {
    header('Location: index.php');
    exit;
} elseif (@$_SESSION['role'] == 'supply') {
    header('Location: index.php');
    exit;
} elseif (@$_SESSION['role'] == 'keuangan') {
    header('Location: visitor/index.php');
    exit;
    ...
    header('Location: visitor/index.php');
    exit;
}elseif (@$_SESSION['role'] == 'ketua_operasional') {
    header('Location: visitor/index.php');
    exit;
}
...
if (isset($_POST['login'])) {
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        $stmtUpdateLastLogin = mysqli_prepare($conn, $updateLastLoginQuery);
        ...
        if ($role == 'admin') {
            ...
            header('Location: visitor/dashboard.php');
            exit;
        } elseif ($role == 'ketua_operasional') {
            $_SESSION['role'] = 'ketua_operasional';
            header('Location: visitor/dashboard.php');
            exit;
...
<head>
    ...
    <style>
        ...
        #backrgoundlogin {
          background: linear-gradient(-45deg, #384B42, #1E5D58, #427D9D);
                    ...
                    <div class="col-lg-7 col-md-10 col-sm-12">
                        <div class="card shadow-lg border-0 rounded-lg" style="margin-top: 100px;">
                            <div class="card-body d-flex">
                                <!-- KIRI GAMBAR -->
                                <div class="col-md-5 d-none d-md-block" style="background-image: url('assets/img/mhg_kantor.png'); background-size: 625px 420px; background-position: 0 0; margin: -20px 0 -20px -20px;">
                                </div>
                                <!-- KANAN KONTEN LOGIN -->
                                <div class="col-md-7 ml-3">
                                    <div class="row">
                                        <div class="col-12 text-center text-md-left">
                                            <img src="assets/logomhg.png" class="mt-1" style="width:100px; display: block;">
                                        </div>
                                        <div class="col-12">
                                            <h5 class="text-left font-weight-bold mt-1" style="color:#427D9D; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2); text-align-justify">PT. MITRA HARUN GASINDO</h5>
                                            <p class="mt-3" style="color: grey; font-size: 13px;">Selamat datang di sistem perizinan dan stok barang PT. Mitra Harun Gasindo</p>
                                            <form method="post">
                                                ...
                                                <div class="form-group d-flex justify-content-end">
                                                    <button class="btn ml-auto" name="login">Login</button>
                                                </div>
