...
$get = mysqli_prepare($conn_aph, "SELECT * FROM legal_people WHERE id_legalpeople=?");
            ...
            <main>
                <div class="container-fluid">
                    ...
                    <a href="../legal_people.php" class="btn btn-danger mb-2 shadow" style="font-size:14px; font-weight: lighter;"><i class="fas fa-arrow-left"></i> </a><br>
                        ...
                        <h1 class="mt-3 text-center mb-4" style="color:#3E578D; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">DATA LEGAL PEOPLE</h1>
                        <div class="card-body">
                            <div class="row mt-5">
                                <h4 style="color:#3E578D; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">Detail</h4>
                            </div>
...
if (isset($_POST['hapus_gambar_people'])) {
    ...
    $result = mysqli_query($conn_aph, "SELECT * FROM dokumen_legal_people WHERE id_berkas_people='$id_berkas_people'");
    $get = mysqli_fetch_array($result);
    $lokgambar = "../lihat_detail/gambar-gambar_people/" . $get['nama_gambar'];
    unlink($lokgambar);

    $hapusgambarpeople = mysqli_query($conn_aph, "DELETE FROM dokumen_legal_people WHERE id_berkas_people='$id_berkas_people'");
...
if (isset($_POST['hapus_berkas_people'])) {
    ...
    $file = mysqli_query($conn_aph, "SELECT * FROM dokumen_legal_people WHERE id_berkas_people='$id_berkas_people'");
    $get = mysqli_fetch_array($file);
    $lokfile = "../lihat_detail/dokumen_people/" . $get['nama_berkas'];
    unlink($lokfile);

    $hapusdokpeople = mysqli_query($conn_aph, "DELETE FROM dokumen_legal_people WHERE id_berkas_people='$id_berkas_people'");
    if ($hapusdokpeople) {
        echo '<script type="text/javascript">      
        Swal.fire({
            position: "center",
