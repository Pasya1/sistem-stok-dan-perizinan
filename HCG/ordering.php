...
<body class="sb-nav-fixed">
    ...
    if ($_SESSION['role'] !== 'admin_hcg' && $_SESSION['role'] !== 'supply_hcg' && $_SESSION['role'] !== 'logistik_hcg') {
            ...
            <main>
                ...
                <div class="container-fluid">
                    ...
                    <div class="">
                            ...
                            <h1 class="mt-3 text-center mb-4" style="color:#FF364C;  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.2);">PERMINTAAN BARANG</h1>
                                    ...
                                    if(isset($_GET['cari'])){ 
                                        ...
                                        if($mulai != null && $selesai != null && $tgl_apa != null){
                                            if($tgl_apa == 'Tanggal_Buat') {
                                                $ambildatapermintaan = mysqli_query($conn_hcg, "SELECT * from permintaan WHERE tanggal_buat BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY)");
                                            } elseif($tgl_apa == 'Tanggal_Permintaan') {
                                                $ambildatapermintaan = mysqli_query($conn_hcg, "SELECT * from permintaan WHERE tanggal_permintaan BETWEEN '$mulai' AND DATE_ADD('$selesai', INTERVAL 1 DAY)");
                                            } else {
                                                $ambildatapermintaan = mysqli_query($conn_hcg, "select * from permintaan");
                                            }
                                        } elseif($mulai != null && $selesai == null && $tgl_apa != null) {
                                            if($tgl_apa == 'Tanggal_Buat') {
                                                $ambildatapermintaan = mysqli_query($conn_hcg, "SELECT * from permintaan WHERE tanggal_buat = '$mulai'");
                                            } elseif($tgl_apa == 'Tanggal_Permintaan') {
                                                $ambildatapermintaan = mysqli_query($conn_hcg, "SELECT * from permintaan WHERE tanggal_permintaan = '$mulai'");
                                            } else {
                                                $ambildatapermintaan = mysqli_query($conn_hcg, "select * from permintaan");
                                            }
                                        } else {
                                            ...
                                            <tr style="background-color: <?php echo ($status_request == 'ACCEPTED') ? 'rgba(0, 255, 0, 0.2)' : (($status_request == 'REJECTED') ? 'rgba(255, 0, 0, 0.2)' : ''); ?>">
                                                ...
                                                <?php
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <button type="button" class="btn dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-expanded="false" style="background-color:#FF364C; height:30px; font-size:12px; color:white;">
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <form method="post">
                                                            ...
                                                            <select name="barangnya" class="form-control" required>
                                                                ...
                                                                $ambilsemuadatanya = mysqli_query($conn_hcg, "select * from stok");
                                                            ...
                                                            <select name="supplier2" class="form-control" required>
                                                                ...
                                                                $ambilsemuadata = mysqli_query($conn_hcg, "select * from supplier");
...
function sendEmail($emailRecipients, $subject, $body) {
    ...
    try {
        ...
        $mail->setFrom('mspasyazakaria@gmail.com', 'PT Harun Citra Gas'); 
...
if(isset($_POST['tambahpermintaan'])) {
    ...
    function generateUniqueTransactionCode($conn_hcg) {
        $tahun = date('Y');
        $bulan = date('m');
    ...
    foreach($tanggalbuat as $key => $value) {
        ...
        $getSupplierInfo = $conn_hcg->prepare("SELECT * FROM supplier WHERE idsupplier = ?");
        ...
        $getbaranginfo = $conn_hcg->prepare("SELECT * FROM stok WHERE idbarang = ?");
        ...
        $kodeTransaksi[$key] = generateUniqueTransactionCode($conn_hcg);
        ...
        $ambilUsername = mysqli_prepare($conn_hcg, "SELECT username, iduser FROM login WHERE role = ?");
...
if (isset($_POST['updatepermintaan'])) {
    ...
    $getSupplierInfo = $conn_hcg->prepare("SELECT namasupplier FROM supplier WHERE idsupplier = ?");
    ...
    $cekstoksekarang = $conn_hcg->prepare("SELECT namabarang FROM stok WHERE idbarang = ?");
    ...
    $updatepermintaan = $conn_hcg->prepare("UPDATE permintaan SET tanggal_buat=?, tanggal_permintaan=?, item_barang=?, qty=?, satuan_bentuk=?, harga_satuan=?, ppn=?, diskon=?, total_harga=?, idsupplier=?, nama_supplier_order=?, keterangan=?, user_edit_order=? WHERE id_order=?");
...
if (isset($_POST['hapuspermintaan'])) {
    ...
    $delete_query2 = $conn_hcg->prepare("DELETE FROM persetujuan_request WHERE idorder = ?");
    ...
    if ($delete_result2) {
        ...
        $delete_query = $conn_hcg->prepare("DELETE FROM permintaan WHERE id_order = ?");
